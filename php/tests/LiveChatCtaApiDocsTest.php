 ...
 */
final class LiveChatCtaApiDocsTest extends TestCase
{
    /** @return string[] "<METHOD> <pattern>" for every route the module mounts */
    private static function mountedRoutes(): array
    {
        // Mounting only needs a container to EXIST — the handlers resolve their
        // repositories lazily, and none of them runs here. This repo has no
        // php-di, so an empty PSR-11 stub stands in (same shape as the one in
        // LiveChatCtaModuleTest).
        ...
        $registry = new ModuleRegistry([new LiveChatCtaModule()]);
    ...
    public function testEveryEntryIsWellFormed(): void
    {
        $permissions = array_map(
            ...
            (new LiveChatCtaModule())->permissions(),
        );

        foreach ((new LiveChatCtaModule())->apiDocs() as $doc) {
